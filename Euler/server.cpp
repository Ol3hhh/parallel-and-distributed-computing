...
[[nodiscard]] int Server::setup(int& serverSocket, sockaddr_in& serverAddress) {
    ...
    // ==============================================================
    // NOWE: Zezwalamy na ponowne użycie portu (omija problem zablokowania po restarcie)
    int opt = 1;
...
float Server::receive(int& clientSocket) { // Zmień zwracany typ w .hpp na float!
    char buffer[1024] = {0};
    ssize_t bytesRead = recv(clientSocket, buffer, sizeof(buffer), 0);
    if (bytesRead > 0) {
        return std::atof(buffer); // Zwraca float zamiast int
    }
    return 0.0f;
}
...
float Server::run() {
    ...
    // Inicjalizacja miejsc na gniazda klientów
    for (int i = 0; i < p; i++) {
        clientSockets.emplace_back();
    }

    int k = 1; // Startujemy od 1, żeby uniknąć 1.0 / 0

    // 1. PĘTLA ROZDAWANIA ZADAŃ
    // Serwer przyjmuje wszystkie p połączeń i natychmiast wysyła zadania
    ...
    // 2. PĘTLA ZBIERANIA WYNIKÓW
    // Serwer nasłuchuje na odpowiedzi od wszystkich p klientów
    for (auto& clientSocket : clientSockets) {
